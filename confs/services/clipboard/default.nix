...
let
  ...
  remoteClipPkg = filterOut: vendorHash:
    let fileset = fs.fileFilter (file: file.name != filterOut) ./.;
...
in {
  ...
  server = remoteClipPkg "client.go"
    "sha256-KMsYgkLRoQvxDJQvqC58zOyUlU2Kc1sx0s0EbDDGIVQ=";
  client = let
    rcClient = remoteClipPkg "server.go"
      "sha256-eDSdZpN51IlQ6TcikkBF0BprOmkjp3+mY5A5VC37RDU=";
  in {
